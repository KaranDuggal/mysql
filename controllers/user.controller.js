...
module.exports = UserController = function () {
    this.signup = async (req, res) => {
        try {
            ...
            if (validUser.error) {
                throw {
                    custom_err_message: "Invalid Schema, in ValidatorServices"
                }
            }
            const checkPhoneNoExist = await userServices.check(req.body.phonenumber)
            ...
            res.json({
                success: false,
                error: err,
                message: err.custom_err_message ? err.custom_err_message : "signup failed"
            })
        }
    }
}